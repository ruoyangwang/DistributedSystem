...
public class JTHandler extends Thread {
		...
		public void check_status(String hash){
			...
			synchronized(JobTracker.zkc){
				...
				List<String> children=JobTracker.zkc.getChildren(JobTracker.WORKER);
				System.out.println("workerchild list size : "+children.size());
				...
				for(String child: Result_children){
					if(hash.equals(child)){
						...
						if(data!=null){
							...
							packetToClient.status = zkPacket.PASSWORD_FAIL;
							try{
								toClient.writeObject(packetToClient);
							}catch(Exception e){
								e.printStackTrace();
