...
public class JobTracker{
	...
	final static String WORKER = "/Workers";
		...
		CurrJobChildWatcher = new Watcher(){
			...
			public void process(WatchedEvent event) {
				...
				if(event.getType()==EventType.NodeChildrenChanged && isPrimary){
					...
					List<String> wkchildren=zkc.getChildren(WORKER);
					//int count = Integer.parseInt(zkc.getData(FILE_SERVER+"/total_workers",null,stat));
					System.out.println("count of Workers now:  "+wkchildren.size()+ "  "+children.size());
					if(children.size()==wkchildren.size()){
						zkc.create(
