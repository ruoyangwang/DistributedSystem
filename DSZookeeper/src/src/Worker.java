...
public class Worker {
    ...
    Stat stat=zkc.exists(Result+"/"+CurJob,null);
    if(stat!=null){
    zkc.setData(Result+"/"+CurJob,answer,-1);
    }
    else
    zkc.create(Result+"/"+CurJob,answer,CreateMode.PERSISTENT);
    List<String> children= zkc.getChildren(CURRENT_JOB+"/"+CurJob,null);
    for(String child : children){
    	zkc.delete(CURRENT_JOB+"/"+CurJob+"/"+child,-1);
    }
