			...
			for(Serialized_Client_Data SCD: eventList){
				if(SCD.Cname.equals(CN)&&SCD.Lamport==packetFromClient.ServerData.Lamport&& SCD.ACK < MazeServer.serverCount){
					...
					if(SCD.ACK == MazeServer.serverCount && SCD.serverHostName.equals(MazeServer.myHostName)){		
						...
						scd.Cname = CN;
						scd.Lamport = SCD.Lamport;
						System.out.println("~~~adding final ack for "+CN+" "+SCD.Lamport);
						try{
							sendQueue.put(scd);
...
*/
	...
	public static void increment_LamportClock(){
		synchronized((Double)MazeServer.LamportClock){
	...
	public static void update_LamportClock(double val){
		synchronized((Double)MazeServer.LamportClock){
		int _val=(int)val;
		MazeServer.LamportClock=(double)_val;
	...
	public synchronized static void print_event_list(){
		synchronized(eventList){
		System.out.print("\n");
		System.out.println("Print event list");
		for(Serialized_Client_Data SCD: eventList){
