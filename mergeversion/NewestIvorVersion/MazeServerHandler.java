...
public class MazeServerHandler extends Thread{
	...
	static volatile ConcurrentHashMap<Double, Integer> 	ACK_Queue=new ConcurrentHashMap<Double, Integer>();
	static volatile BlockingQueue<Serialized_Client_Data> sendQueue = new ArrayBlockingQueue<Serialized_Client_Data>(1000);
	static Object _scd=new Object();
	...
	public void run() {
		...
		try{		
				...
				while (( packetFromClient = (MazePacket) from_Client.readObject()) != null) {
					...
					/*
					synchronized (ACK_Queue){
					if(packetFromClient.ServerData != null){
						if(ACK_Queue.get(packetFromClient.ServerData.Lamport)!=null&&packetFromClient.ServerData.event!=MazePacket.ACK){
						packetFromClient.ServerData.ACK+=ACK_Queue.get(packetFromClient.ServerData.Lamport);
						System.out.println("Adding lock ack back "+packetFromClient.ServerData.Lamport+" "+packetFromClient.ServerData.ACK);
						ACK_Queue.remove(packetFromClient.ServerData.Lamport);
						}
					}
					*/
					//}
		...
		while(waitForEvent){
			...
			for(Serialized_Client_Data SCD: eventList){
				...
				if(SCD.Cname.equals(CN)&&SCD.Lamport==packetFromClient.ServerData.Lamport&& SCD.ACK <MazeServer.serverCount){
					...
					//System.out.print("eventlist peek \n"+eventList.peek().Lamport+"\n"+"ack: "+eventList.peek().ACK+"\n");
					break;
...
/*	
	...
	private synchronized void add_sort_eventlist(Serialized_Client_Data ServerData){
		synchronized (eventList){
		for(Serialized_Client_Data SCD: eventList){
			if(SCD.Lamport == ServerData.Lamport ){
					if(SCD.pid > ServerData.pid){
						increment_LamportClock();		
						SCD.Lamport = MazeServer.LamportClock;							
									
						}
					else{
						packetFromClient.ServerData.Lamport+= 1;	
						}
...
*/
	...
	public static synchronized void add_One_Event(Serialized_Client_Data Data){
		//synchronized(eventList){
